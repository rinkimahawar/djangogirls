...
from .serializer import UserSerializer, UserSerializers, PostSerializer,CommentSerializer,CategorySerializer,TagsSerializer
from rest_framework.authtoken.models import Token


class SignupViewSett(viewsets.ModelViewSet):
...
class postViewSet(viewsets.ModelViewSet):
	...
	def create(self, request, *args, **kwargs):
		...
		if serializer.is_valid():
			serializer.save()
			return Response(data=serializer.data, status=tstatus.HTTP_201_CREATED)
		else:
			return Response(data=serializer.errors, status=tstatus.HTTP_400_BAD_REQUEST)
...
class CommentViewSet(viewsets.ModelViewSet):
	...
	# 		}
		serializer = self.serializer_class(data=data,context={'post': post})
		if serializer.is_valid():
			serializer.save()
			return Response({"message": "comment successfully.","data":data}, status=tstatus.HTTP_200_OK)
		else:
			return Response({"message": "invalid comment."}, status=tstatus.HTTP_400_BAD_REQUEST)
...
class CategoryViewSet(viewsets.ModelViewSet):
	...
	# 	if serializer.is_valid():
	# 		serializer.save()
	# 		return Response({"message": "Category successfully.","data":data}, status=tstatus.HTTP_200_OK)
	# 	else:
	# 		return Response({"message": "invalid Category."}, status=tstatus.HTTP_400_BAD_REQUEST)		
...
class  TagsViewSet(viewsets.ModelViewSet):
	...
	# 	if serializer.is_valid():
	# 		serializer.save()
	# 		return Response({"message": "Tags successfully.","data":data}, status=tstatus.HTTP_200_OK)
	# 	else:
	# 		return Response({"message": "invalid Tags."}, status=tstatus.HTTP_400_BAD_REQUEST)		
...
class UserViewSet(viewsets.ModelViewSet):
	...
	def create(self, request, *args, **kwargs):
		...
		try:
			...
			# if email is None:
			# 	res['status'] = False
			# 	res['message'] = "email already created"
			# 	res['data'] = []
			# 	return Response(res, status=tstatus.HTTP_400_BAD_REQUEST)
			# if username is None:
			# 	res['status'] = False
			# 	res['message'] = "username already created"
			# 	res['data'] = []
			# 	return Response(res, status=tstatus.HTTP_400_BAD_REQUEST)				
		except:
			pass		
		...
		if serializer.is_valid():
			serializer.save()
			return Response({"message": "user created successfully.",'data':data}, status=tstatus.HTTP_201_CREATED)
		else:
			return Response({"message": "invalid user.",'data':serializer.errors}, status=tstatus.HTTP_400_BAD_REQUEST)
